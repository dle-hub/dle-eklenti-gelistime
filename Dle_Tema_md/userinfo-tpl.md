...
The output of this section is configured in the userinfo.tpl file. This section configures the templates used when displaying the user profile. Editing this section is not recommended without HTML knowledge because field names are used that are passed to the script through the form. In other words, you may edit the text as you wish, but do not change the field names, this is extremely important. The following tags can be used:

#### TAG: `{user-id}`

Outputs the user ID. This tag can be used in a number of cases when designing the template and also, for example, for creating custom links such as links to the user's latest comments, and so on.
...
Outputs the number of comments

#### TAG: `{news-num}`

Outputs the number of the user's news items

#### TAG: `{status}`

User status (their group on the website)
...
Output the enclosed text if the first rating type 'Score' is enabled for comments in the script settings.
...
Outputs the date of the user's registration on the website

#### TAG: `{editinfo}`

Outputs the existing short information into the field when editing the profile

#### TAG: `{hidemail}`

Outputs the checkbox for hiding the e-mail address.
...
Outputs the user's place of residence
`[own-profile]`
text
[/own-profile]
...
Outputs the code enclosed in the tags if the user viewing the profile is this very user or the website administrator. It is used so that visitors can edit their own profile.
`[signature]`
text
[/signature]
...
Outputs the user's signature formatted for editing
...
Outputs a link to all comments of this user

#### TAG: `{allowed-ip}`

The IP value from which authorization is allowed

#### TAG: `{xfields}`

Outputs the list of profile additional fields when editing them
`[xfinput_X]`
...
Outputs the text specified in the tags if the additional field "x" is not empty.
`[xfnotgiven_x]`
text
[/xfnotgiven_x]
...
Outputs the text between the tags if the user is in a temporary group

#### TAG: `{time_limit}`

Outputs the date until which the user remains in the temporary group

#### TAG: `{news}`

Outputs a link to view all news of this user
`[rss]`
text
[/rss]

Outputs a link in the profile to the RSS feed of all user news.

#### TAG: `{ignore-list}`

List of users added to ignored users.
`[fullname]`
text
[/fullname]
...
Outputs the text enclosed in the tags if the user has not entered short information about themselves in the profile
`[news-num]`
text
[/news-num]
...
Outputs the enclosed text if the group of the user whose profile is being viewed belongs to the specified group X, where X is the group number. Listing several required groups separated by commas is also allowed.
...
Outputs the list of time zones available in the system and allows the user to choose their own time zone for their profile on the website.

#### TAG: `{unsubscribe}`

Outputs the checkbox for unsubscribing from comments and the number of subscribed publications.

#### TAG: `{news-subscribe}`

Outputs the checkbox for subscribing to notifications about new comments added to news items whose author is the user.

#### TAG: `{comments-reply-subscribe}`

Outputs the checkbox for subscribing to notifications about new comments that are replies to the user's own comments.

#### TAG: `{pm}`

Link to write a personal message to the user

#### TAG: `{email}`

Link to write an E-Mail message to the user

#### TAG: `{twofactor-auth}`

Outputs the checkbox for enabling or disabling two-factor authentication on the website for the user
`[banned]`
text
[/banned]
...
Outputs the list of linked social networks and links to disconnect them from this account
`[attached-vk]`
text
[/attached-vk]

Outputs the enclosed text if the user's account is linked to the VK network
`[attached-odnoklassniki]`
text
[/attached-odnoklassniki]

Outputs the enclosed text if the user's account is linked to the Odnoklassniki network
`[attached-facebook]`
text
[/attached-facebook]

Outputs the enclosed text if the user's account is linked to the Facebook network
`[attached-google]`
text
[/attached-google]

Outputs the enclosed text if the user's account is linked to the Google network
`[attached-mailru]`
text
[/attached-mailru]

Outputs the enclosed text if the user's account is linked to the Mail.ru network
`[attached-yandex]`
text
[/attached-yandex]

Outputs the enclosed text if the user's account is linked to the Yandex network
`[detach-vk]`
text
[/detach-vk]

Outputs the enclosed text as a link to detach the account from the VK social network if the user's account is linked to it.
`[detach-odnoklassniki]`
text
[/detach-odnoklassniki]

Outputs the enclosed text as a link to detach the account from the Odnoklassniki social network if the user's account is linked to it.
`[detach-facebook]`
text
[/detach-facebook]

Outputs the enclosed text as a link to detach the account from the Facebook social network if the user's account is linked to it.
`[detach-google]`
text
[/detach-google]

Outputs the enclosed text as a link to detach the account from the Google social network if the user's account is linked to it.
`[detach-mailru]`
text
[/detach-mailru]

Outputs the enclosed text as a link to detach the account from the Mail.ru social network if the user's account is linked to it.
`[detach-yandex]`
text
[/detach-yandex]

Outputs the enclosed text as a link to detach the account from the Yandex social network if the user's account is linked to it.
`[ignore]`
text
[/ignore]
...
Outputs the enclosed text as a link for the user to delete their own profile on the website if this is allowed by the user group settings
`[if field = "value"]`
text
[/if]
...
logged_ip - IP from which the user logged into the website (text)
